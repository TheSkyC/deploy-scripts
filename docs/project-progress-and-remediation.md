...
> **审计基准**：P0-S2 安全审计代码基线为 `af6b511`（2026-08-30）；当前整改代码基线为 `d51b4cc`（TickFlow 固定 git 提交发布产物）。
...
| 当前整改代码基线 | `d51b4cc` | TickFlow 固定 git 提交及其发布产物；后续文档提交不改变该代码基线 |
...
- CyberStrikeAI：git branch 模式，branch 会移动，尚非 commit pin；
...
- [ ] 为 Vaultwarden/CyberStrikeAI/CPA Stack/Sub2API 定义统一版本记录字段；Hugo 已具备 `HUGO_VERSION`、`INSTALLED_VERSION` 和 GitHub release tag 语义，TickFlow 已具备 `TICKFLOW_COMMIT`、`INSTALLED_VERSION` 和 git commit pin 语义。后续优先支持不可变镜像 digest 和组件版本 manifest。
