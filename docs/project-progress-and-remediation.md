...
> **审计基准**：P0-S2 安全审计代码基线为 `af6b511`（2026-08-30）；当前整改代码基线为 `c82c267`（Sub2API 真实 PostgreSQL/Redis E2E fixture quoting repair）。
...
| 当前整改代码基线 | `96f160e` / `c82c267` | Sub2API PostgreSQL 密码变量修复及真实依赖 E2E fixture quoting repair；后续文档提交不改变代码基线 |
...
- [ ] 提取 Sub2API、Vaultwarden、CyberStrikeAI 共同的“配置/锁/回滚/健康/备份”原语，继续减少复制，但保持应用特有分发和数据模型。已完成若干边界明确的小步：
  ...
  提示语义。Sub2API 独立 cron 备份脚本的归档发布已收敛为单一本地
  `_publish_backup_artifact`（私有 0600 + sha256 sidecar），消除生成模板内的三份内联
  重复；仍待继续审查配置/锁、回滚编排、PostgreSQL/sidecar 语义及独立 cron 脚本的
  适配边界，不能将整项提前标记为完成。
