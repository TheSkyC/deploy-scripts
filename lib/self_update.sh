...
self_update_load_config() {
  ...
  if [[ -f "$config_file" ]]; then
    load_config_file "$config_file" \
      ...
      DEPLOY_SELF_UPDATE_REQUIRE_SIGNATURE DEPLOY_SELF_UPDATE_PUBLIC_KEY \
...
self_update_main() {
  local json=0 check=0 channel_override="" arg
  while (($#)); do
    arg="$1"
    case "$arg" in
      --json) json=1; shift ;;
      ...
      --dry-run|--rollback|--list) printf 'self-update: %s is not implemented yet\n' "$arg" >&2; return 2 ;;
      -h|--help) printf '%s\n' 'Usage: deploy.sh self-update --check [--json] [--channel NAME]' >&2; return 0 ;;
