...
- ✅ `feat(beszel): add GitHub-release binary deployment` — Beszel Hub 通过 tar.gz 发行包接入共享二进制生命周期；数据目录为 `/var/lib/beszel`，服务端口默认 8090。
...
- ✅ 审计复核：9 个二进制应用（ntfy/meilisearch/alist/filebrowser/navidrome/frps/gitea/gotify/beszel）的 lib + impl + apps + checks 全部审阅；i18n `%s` 与传参逐一比对（Python 解析器）未发现剩余不匹配。
...
- miniflux / adguard / minio / listmonk / syncthing 未纳入本轮队列（性价比未排到；miniflux 资产未核实）。
...
2. 每个新应用一个 feat commit（apps/impl/bin/wrapper/registry/checks + 重新生成的 dist）：
   ...
   - `feat(navidrome): ...` / `feat(frps): ...` / `feat(gitea): ...` / `feat(gotify): ...` / `feat(beszel): ...`
