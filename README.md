...
For New API and Sub2API automation, `DEPLOY_ASSUME_YES=1` confirms uninstall without prompts while keeping data, config, and backups by default. Add `DEPLOY_DELETE_DATA=1`, `DEPLOY_DELETE_CONFIG=1`, or `DEPLOY_DELETE_BACKUP=1` only when those removals are intended.
