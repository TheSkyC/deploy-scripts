...
check_localized_dispatch() {
  ...
  expect_failure_output en install_newapi.sh "does not support cert" cert
  expect_failure_output zh install_newapi.sh "暂不支持 cert" cert
  expect_failure_output en dist/install_newapi.sh "does not support cert" cert
  expect_failure_output zh dist/install_newapi.sh "暂不支持 cert" cert
}
...
check_app_localized_descriptions() {
  ...
  expect_app_description newapi en "New API deployment with systemd, backups, and operational checks."
  expect_app_description newapi zh "使用 systemd、备份和运维检查的 New API 部署脚本。"
  ...
  expect_app_description navidrome en "Navidrome music server deployment with systemd and backups."
  ...
  expect_app_description beszel en "Beszel monitoring hub deployment with systemd and backups."
  expect_app_description beszel zh "使用 systemd 和备份的 Beszel 监控中心部署脚本。"
}
