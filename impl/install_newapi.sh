...
# 崩溃后自动重启（最多每 60 秒重启 5 次，避免 crash loop 把机器打满）
Restart=always
RestartSec=5
...
# SSE 流式响应优化：关闭 Go DNS 的 CGO 解析，避免超时卡顿
...
# 允许写入的路径：数据目录 + 日志目录（其余路径均为只读）
ReadWritePaths=${DATA_DIR} ${LOG_DIR}

# 日志：输出到 systemd journal，可通过 journalctl -u ${SERVICE_NAME} 查看
...
${LOG_DIR}/*.log {
    ...
    missingok
    notifempty
    # copytruncate：原地截断日志文件，无需服务支持 SIGHUP
    # 缺点：极短窗口内可能丢失少量日志行，对 API 网关场景可接受
    copytruncate
}
LOGR
...
# 重新生成：sudo bash install_newapi.sh install
set -euo pipefail
umask 077
...
# 原子备份：先写 .tmp，成功后 mv，避免中途失败留下残破存档
...
show_menu() {
  ...
  echo ""
  echo -e "  ${CYAN}1)${NC} ${BOLD}install${NC}    — 全新安装（下载二进制 + systemd + 日志轮转 + 定时备份）"
  echo -e "  ${CYAN}2)${NC} ${BOLD}update${NC}     — 更新到最新版（自动备份 → 下载 → 替换 → 失败自动回滚）"
  echo -e "  ${CYAN}3)${NC} ${BOLD}backup${NC}     — 立即手动备份数据（含 SQLite WAL flush + 完整性校验）"
  echo -e "  ${CYAN}4)${NC} ${BOLD}status${NC}     — 查看全面状态（服务/版本/进程资源/健康检查/防火墙）"
  echo -e "  ${CYAN}5)${NC} ${BOLD}uninstall${NC}  — 卸载（交互式确认，可选保留数据/备份）"
  echo -e "  ${CYAN}q)${NC} 退出"
  echo ""
  prompt "请输入选项 [1-5/q]："
  local MENU_CHOICE
...
case "${1:-menu}" in
  ...
  menu|"")   show_menu    ;;
  *)
    echo -e "用法：sudo bash $0 [install|update|backup|status|uninstall]" >&2
    echo -e "      不带参数则显示交互式菜单" >&2
    exit 1
    ;;
esac
fi
