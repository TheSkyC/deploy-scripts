...
show_menu() {
  ...
  echo ""
  echo -e "  ${CYAN}1)${NC} ${BOLD}install${NC}    — 全新安装（PG 15 + Redis 7 + Nginx + 二进制 + systemd + 备份）"
  echo -e "  ${CYAN}2)${NC} ${BOLD}update${NC}     — 更新到最新版（自动备份 → 下载 → 校验 → 替换 → 失败自动回滚）"
  echo -e "  ${CYAN}3)${NC} ${BOLD}backup${NC}     — 立即手动备份（pg_dump + 配置目录）"
  echo -e "  ${CYAN}4)${NC} ${BOLD}status${NC}     — 查看全面状态（服务 / Nginx / 依赖 / 健康检查 / 防火墙）"
  echo -e "  ${CYAN}5)${NC} ${BOLD}uninstall${NC}  — 卸载（交互确认，可选保留数据 / 配置 / 备份）"
  echo -e "  ${CYAN}q)${NC} 退出"
  echo ""
  prompt "请输入选项 [1-5/q]："
  local MENU_CHOICE
